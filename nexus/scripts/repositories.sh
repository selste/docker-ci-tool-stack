...
do
  ...
  status=$(curl -s -o /dev/null -w '%{http_code}' -u admin:admin123 --header 'Content-Type: application/json' 'http://localhost:18081/service/siesta/rest/v1/script/' -d @$script)
...
do
  ...
  status=$(curl -s -o /dev/null -w '%{http_code}' -X POST -u admin:admin123 --header 'Content-Type: text/plain' "http://localhost:18081/service/siesta/rest/v1/script/$alias/run")
